...
export default function JournalPage() {
  ...
  return (
    <div className="flex flex-col items-center h-full p-6 overflow-auto relative">
      {/* Header */}
      <div className="w-full max-w-2xl flex items-center justify-between mb-8 animate-in relative z-10">
        <button onClick={() => cloudNav(() => { setScreen('era-map'); r.push('/era-select'); })} className="flex items-center gap-1 text-sm text-white/60 hover:text-white transition-colors font-medium">
        ...
        <span className="text-xs text-white/50 font-medium">{totalDiscovered} 个发现</span>
      </div>

      <h1 className="text-2xl font-serif font-bold text-white/95 mb-2 animate-in animate-in-d1 relative z-10">📖 声音图鉴</h1>
      <p className="text-xs text-white/50 mb-8 animate-in animate-in-d1 relative z-10">Sound Journal — 已发现的声音碎片</p>

      {phonemeArr.length === 0 && letterArr.length === 0 ? (
        <div className="text-center mt-20 animate-in relative z-10">
          <span className="text-5xl">📓</span>
          <p className="text-white/40 mt-4">还没有任何发现</p>
          <p className="text-xs text-white/20">完成关卡来收录音素</p>
        </div>
      ) : (
        <div className="flex gap-8 max-w-2xl w-full flex-wrap justify-center relative z-10">
          {/* Phoneme cards */}
            ...
            <div className="flex flex-wrap gap-3">
              {phonemeArr.map(p => (
                <button key={p} onClick={() => setSelected(selected === p ? null : p)}
                  className={`relative px-4 py-3 rounded-xl transition-all duration-200 text-left ${selected === p ? 'bg-white/20 shadow-md scale-105 ring-2 ring-[#f59e0b]/30' : 'bg-white/10 hover:bg-white/20 backdrop-blur-sm'}`}>
                  <span className="text-lg font-bold text-white/90">/{p}/</span>
          ...
          {letterArr.length > 0 && (
            <div className="flex-1 min-w-[150px]">
              ...
              <div className="flex flex-wrap gap-3">
                {letterArr.map(l => (
                  <div key={l}
                    className="px-5 py-3 rounded-xl bg-white/10 backdrop-blur-sm text-center">
                    <span className="text-2xl font-serif font-bold text-white/90">{l}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* Detail panel */}
      {selectedInfo && (
        <div className="mt-6 p-5 rounded-2xl bg-white/15 backdrop-blur-md max-w-sm w-full animate-in border border-white/10 relative z-10">
          <h3 className="text-lg font-bold text-white/90 mb-2">/{selected}/</h3>
          <p className="text-sm text-white/70 leading-relaxed mb-3">{selectedInfo.desc}</p>
