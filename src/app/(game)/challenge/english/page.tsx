  ...
  return (
    <div className="min-h-screen flex flex-col items-center">
      <div className="w-full max-w-2xl px-6 pt-6">
        <button onClick={()=>r.push('/challenge')} className="text-xs text-white/50 hover:text-[#d4912a] mb-4">← 返回</button>
        {phase==='setup'&&(
          <div className="flex flex-col items-center gap-8 animate-in">
            <h1 className="font-display text-3xl font-bold text-white/95">⚡ 英语挑战</h1>
            <div className="w-full space-y-5">
              <div><p className="text-xs text-white/50 mb-2 uppercase tracking-wider">题目数量</p><div className="flex gap-2">{[4,8,12,16].map(n=><button key={n} onClick={()=>setQCount(n)} className={`px-4 py-2 rounded-xl text-sm font-bold ${qCount===n?'bg-[#d4912a] text-[#0f0d0a]':'bg-white/10 text-white/50'}`}>{n}题</button>)}</div></div>
              <div><p className="text-xs text-white/50 mb-2 uppercase tracking-wider">总时间</p><div className="flex gap-2">{[{v:60,l:'60秒'},{v:90,l:'90秒'},{v:120,l:'2分钟'}].map(o=><button key={o.v} onClick={()=>setTotalTime(o.v)} className={`px-4 py-2 rounded-xl text-sm font-bold ${totalTime===o.v?'bg-[#d4912a] text-[#0f0d0a]':'bg-white/10 text-white/50'}`}>{o.l}</button>)}</div></div>
              <div><p className="text-xs text-white/50 mb-2 uppercase tracking-wider">题型</p><div className="flex gap-2">{(['phonics','spell','memory']as QType[]).map(t=>{const l:Record<QType,string>={phonics:'🔊辨音',spell:'✍️拼写',memory:'🧠记忆'};return <button key={t} onClick={()=>setEnabledTypes(p=>p.includes(t)?p.filter(x=>x!==t):[...p,t])} className={`px-4 py-2 rounded-xl text-sm font-bold ${enabledTypes.includes(t)?'bg-[#d4912a] text-[#0f0d0a]':'bg-white/10 text-white/50'}`}>{l[t]}</button>;})}</div></div>
            </div>
        ...
        {phase==='playing'&&q&&(
          <div className="flex flex-col items-center flex-1 w-full">
            <div className="w-full flex items-center justify-between mb-4"><div className="flex items-center gap-4"><span className="text-[#d4912a] font-mono text-lg font-bold">{qIndex+1}/{questions.length}</span><span className="text-sm text-white/50">⭐{score}</span>{streak>=3&&<span className="text-xs text-[#f97316]">🔥x{streak}</span>}</div><div className={`font-mono text-lg font-bold ${timeLeft<=10?'text-[#ef4444]':'text-white/95'}`}>{timeLeft}s</div></div>
            <div className="w-full h-2 rounded-full bg-white/10 mb-8 overflow-hidden"><div className={`h-full rounded-full transition-all ${timeLeft<=10?'bg-[#ef4444]':'bg-[#d4912a]'}`} style={{width:`${(timeLeft/totalTime)*100}%`}}/></div>
            <div className={`flex-1 flex flex-col items-center justify-center w-full rounded-3xl transition-all min-h-[300px] ${showFeedback==='correct'?'bg-[#10b981]/10 border-2 border-[#10b981]/30':showFeedback==='wrong'?'bg-[#ef4444]/10 border-2 border-[#ef4444]/30':'bg-white/10 border-2 border-transparent'}`}>
              {q.type==='phonics'?(
                <div className="flex flex-col items-center gap-6"><button onClick={()=>speak(`/${q.word}/`)} className="w-20 h-20 rounded-full bg-[#d4912a]/10 border-2 border-[#d4912a]/30 flex items-center justify-center text-3xl hover:scale-110 transition-transform">🔊</button><p className="text-white/50 text-sm">听到的是哪个音素？</p><div className="flex gap-4">{q.options?.map(o=><button key={o} onClick={()=>check(o)} className="w-16 h-16 rounded-2xl bg-white/10 border-2 border-white/10 text-2xl font-bold text-white/95 hover:border-[#d4912a] transition-all">{o}</button>)}</div></div>
              ):(
                <div className="flex flex-col items-center gap-6">{flashWord&&<p className="text-5xl font-extrabold text-[#f97316] font-display bouncy-pop">{flashWord.toUpperCase()}</p>}{!flashWord&&<button onClick={()=>speak(q.word)} className="w-16 h-16 rounded-full bg-[#d4912a]/10 border-2 border-[#d4912a]/30 flex items-center justify-center text-2xl hover:scale-110 transition-transform">🔊</button>}<p className="text-white/50 text-xs">{q.type==='memory'?'记住然后拼出来':'听发音拼出来'}</p><div className="flex gap-2">{Array.from({length:q.word.length}).map((_,i)=><button key={i} onClick={()=>setSlots(s=>s.filter((_,j)=>j!==i))} className={`w-12 h-14 rounded-xl border-2 flex items-center justify-center text-xl font-bold ${slots[i]?'bg-[#d4912a]/10 border-[#d4912a]/30 text-[#d4912a]':'bg-white/10 border-white/10'}`}>{slots[i]?.toUpperCase()||''}</button>)}</div><div className="flex gap-2 flex-wrap justify-center max-w-sm">{shuffle([...q.word.split(''),...'abcdefghijklmnopqrstuvwxyz'.split('').filter(l=>!q.word.includes(l)).slice(0,Math.max(0,8-q.word.length))]).map((l,i)=><button key={i} onClick={()=>{if(slots.length>=q.word.length)return;const ns=[...slots,l];setSlots(ns);if(ns.length===q.word.length)check(ns.join(''));}} disabled={slots.filter(x=>x===l).length>=q.word.split('').filter(x=>x===l).length} className="w-10 h-10 rounded-xl bg-white/10 border border-white/10 text-sm font-bold text-white/95 hover:border-[#d4912a] transition-all disabled:opacity-20">{l.toUpperCase()}</button>)}</div></div>
              )}
            </div>
        ...
        {phase==='result'&&(
          <div className="flex flex-col items-center justify-center gap-6 py-12 animate-in">
            <div className="w-20 h-20 rounded-full bg-[#d4912a]/10 border-2 border-[#d4912a]/30 flex items-center justify-center text-3xl">🏆</div><h1 className="font-display text-3xl font-bold text-white/95">挑战完成!</h1>
            <div className="flex gap-8 text-center"><div><p className="text-3xl font-bold text-[#d4912a]">{score}</p><p className="text-xs text-white/50">得分</p></div><div><p className="text-3xl font-bold text-white/95">{questions.length}</p><p className="text-xs text-white/50">题数</p></div><div><p className="text-3xl font-bold text-[#10b981]">{streak}</p><p className="text-xs text-white/50">连击</p></div></div>
