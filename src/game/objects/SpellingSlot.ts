...
export class SpellingSlot extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, size: number, index: number) {
    ...
    // Highlight ring
    this.highlight = scene.add.rectangle(0, 0, size + 6, size + 6, 0x444444, 0);
    this.highlight.setStrokeStyle(2, 0x444444, 0);
    this.add(this.highlight);

    // Slot background
    this.bg = scene.add.rectangle(0, 0, size, size, randomDarkFill(), 0.9);
    this.bg.setStrokeStyle(2, 0x444444, 0.3);
    this.add(this.bg);

    // Letter text
    this.letterText = scene.add.text(0, 0, '', {
      fontSize: '36px', color: '#000000', fontFamily: 'Crimson Text, serif', fontStyle: 'bold',
    }).setOrigin(0.5);
    this.add(this.letterText);

    // Dashed bottom line
    const dash = scene.add.graphics();
    dash.lineStyle(2, 0x444444, 0.2);
  ...
  clear() {
    ...
    this.bg.setStrokeStyle(2, 0x444444, 0.3);
    this.bg.setFillStyle(0x1a1a1a, 0.9);
  }

  showHighlight() {
    this.highlight.setStrokeStyle(2, 0x444444, 0.5);
    this.highlight.setFillStyle(0x444444, 0.05);
  }

  hideHighlight() {
    this.highlight.setStrokeStyle(2, 0x444444, 0);
    this.highlight.setFillStyle(0x444444, 0);
  }
}
